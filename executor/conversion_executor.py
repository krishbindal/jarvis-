...
def convert_to_mp3(file_path: str) -> Dict:
    try:
        ...
        if result.returncode != 0:
            return {"success": False, "message": f"ffmpeg failed: {result.stderr.strip()}"}
        return {"success": True, "message": f"Converted to MP3: {dest}", "path": str(dest)}
...
def convert_to_pdf(file_path: str) -> Dict:
    try:
        ...
        return {"success": True, "message": f"Converted to PDF: {dest}", "path": str(dest)}
    except Exception as exc:  # noqa: BLE001
        return {"success": False, "message": f"Conversion failed: {exc}"}
